...
void setup() {
  ...
  lcm = lcm_alpha * width;
  file_c = createWriter("risultati_2c.csv");
...
// UNA VOLTA CHE IL SISTEMA PER SALVARE I DATI E' PRONTO (DA FARE QUANDO MUORE IL FOTONE NON QUANDO FINISCONO TUTTI) QUI PUO RIMANERE SOLO exit();
  if (fotoni.isEmpty()) {
    ...
    //SALVO I DATI NEL FILE C
    file_c.println(quantita_fotoni + "," + quantita_scappati);

    //CHIUDO I FILE APERTI E TERMINO IL PROGRAMMA
    file_c.flush();
    file_c.close();
    exit();
  }
